...
from openparf.ops.wasll import wasll
...
def buildWirelength(
    pin_x, pin_y, pin2net_map, net2pin_map, gamma, ignore_net_degree,
    net_weights
):
    """
    ...
    # sum of exp(x)
    sum_exp_pin_x = unsortedSegmentSum(exp_pin_x, pin2net_map, len(net2pin_map))
    sum_exp_pin_y = unsortedSegmentSum(exp_pin_y, pin2net_map, len(net2pin_map))
    sum_nexp_pin_x = unsortedSegmentSum(
        nexp_pin_x, pin2net_map, len(net2pin_map)
    )
    sum_nexp_pin_y = unsortedSegmentSum(
        nexp_pin_y, pin2net_map, len(net2pin_map)
    )

    # sum of x*exp(x)
    sum_x_exp_pin_x = unsortedSegmentSum(
        pin_x * exp_pin_x, pin2net_map, len(net2pin_map)
    )
    sum_y_exp_pin_y = unsortedSegmentSum(
        pin_y * exp_pin_y, pin2net_map, len(net2pin_map)
    )
    sum_x_nexp_pin_x = unsortedSegmentSum(
        pin_x * nexp_pin_x, pin2net_map, len(net2pin_map)
    )
    sum_y_nexp_pin_y = unsortedSegmentSum(
        pin_y * nexp_pin_y, pin2net_map, len(net2pin_map)
    )
...
class WASLLOpTest(unittest.TestCase):

    def testWASLLRandom(self):
        num_slrX, num_slrY = 2, 2
        # suppose pin_pos is the transformation by the soft_floor method
        pin_pos = np.array(
            [[0.0, 0.0], [1.0, 1.0], [1.5, 0.2], [0.5, 1.9], [0.6, 1.1]],
            dtype=np.float32
        )
        ...
        golden_value = np.array(
            [
                buildWirelength(
                    pin_x, pin_y, pin2net_map, net2pin_map, gamma,
        ...
        pin_pos_var = Variable(
            torch.tensor(pin_pos, dtype=torch.float32).reshape([-1]),
            requires_grad=True
        )
        print(pin_pos_var)
        ...
        if configure.compile_configurations[
            ...
            custom_cuda = wasll.WASLL(
                ...
                gamma=torch.tensor(gamma, dtype=torch.float32).cuda(),
                num_slrX=num_slrX,
                num_slrY=num_slrY
            )
            ...
            np.testing.assert_allclose(
                result_cuda.data.cpu().numpy(), result.data.numpy(), atol=1e-6
            )
            np.testing.assert_allclose(
                grad_cuda.data.cpu().numpy(),
                grad.data.numpy(),
                rtol=1e-6,
                atol=1e-6
            )
