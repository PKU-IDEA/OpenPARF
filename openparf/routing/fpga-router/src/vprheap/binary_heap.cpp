...
void BinaryHeap::init_heap(std::shared_ptr<router::RouteGraph> graph) {
    size_t target_heap_size = graph->getWidth() * graph->getHeight();
    if (heap_ == nullptr || heap_size_ < target_heap_size) {
        ...
        heap_size_ = graph->getWidth() * graph->getHeight();
...
void BinaryHeap::prune_heap() {
    assert(max_index_ < prune_limit_);
    ...
    for (size_t i = 1; i < heap_tail_; i++) {
        ...
        assert(static_cast<size_t>(heap_[i]->index) < max_index_);
