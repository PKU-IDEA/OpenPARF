...
template <typename T>
__global__ void computeWASLL(T const *pos, int32_t const *flat_netpin,
  ...
  if (i >= num_nets || (!net_mask[i])) return;
  if ((xy == 0 && num_slrX == 1) || (xy == 1 && num_slrY == 1)) return;
...
void computeWASLLCudaLauncher(T const *pos, int32_t const *flat_netpin,
                              ...
                              T *grad_intermediate) {
  // separate x and y
  computeWASLL<T><<<ceilDiv(num_nets, 64), {64, 2}>>>(
      pos, flat_netpin, netpin_start, net_mask, num_nets, num_slrX, num_slrY,
      inv_gamma, partial_sll, grad_intermediate);
}

template <typename T>
__global__ void integrateNetWeights(int32_t const *pin2net_map,
