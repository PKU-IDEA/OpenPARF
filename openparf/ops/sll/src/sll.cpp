...
template <typename T>
void computeSLLLauncher(T const *pos, int32_t const *flat_netpin,
                        int32_t const *netpin_start, uint8_t const *net_mask,
                        int32_t *sll_counts_table, int32_t *partial_sll, 
                        int32_t num_nets, T xl, T yl, T slr_width,
...
at::Tensor sllForward(at::Tensor pos, at::Tensor flat_netpin,
                      ...
                      double xl, double yl, double slr_width, double slr_height,
  ...
  CHECK_FLAT_CPU(sll_counts_table);
  ...
  OPENPARF_DISPATCH_FLOATING_TYPES(pos, "computeSLLLauncher", [&] {
    computeSLLLauncher<scalar_t>(
        ...
        slr_width, slr_height, num_slrX, num_slrY, at::get_num_threads());
  });
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &OPENPARF_NAMESPACE::sllForward,
        "super long lines counts forward computation");
}