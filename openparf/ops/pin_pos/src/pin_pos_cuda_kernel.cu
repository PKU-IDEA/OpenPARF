...
template <typename T>
__global__ void computePinPos(T const* pos, T const* pin_offsets,
...
void computePinPosCudaLauncher(T const* pos, T const* pin_offsets,
                               int32_t const* pin2node_map, int32_t num_pins,
                               T* pin_pos) {
  computePinPos<<<ceilDiv(num_pins, 256), 256>>>(pos, pin_offsets, pin2node_map,
                                                 num_pins, pin_pos);
}

template <typename T>
__global__ void computePinPosGrad(T const* grad_outs, int32_t const* node_pins,
...
void computePinPosGradCudaLauncher(T const* grad_outs, int32_t const* node_pins,
                                   int32_t const* node_pins_start,
                                   int32_t num_nodes, T* grads) {
  computePinPosGrad<<<ceilDiv(num_nodes, 256), 256>>>(
      grad_outs, node_pins, node_pins_start, num_nodes, grads);
}

// manually instantiate the template function
#define REGISTER_KERNEL_LAUNCHER(T)                                                                \
    template void computePinPosCudaLauncher<T>(T const *pos, T const *pin_offsets,                 \
                                               int32_t const *pin2node_map, int32_t num_pins,      \
                                               T *pin_pos);                                        \
    template void computePinPosGradCudaLauncher<T>(T const *grad_outs, int32_t const *node_pins,   \
                                                   int32_t const *node_pins_start,                 \
                                                   int32_t num_nodes, T *grads);
