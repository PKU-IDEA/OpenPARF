...
template <typename T>
void computePinPosLauncher(T const* pos, T const* pin_offsets,
                           int32_t const* pin2node_map, int32_t num_pins,
                           int32_t num_threads, T* pin_pos);

template <typename T>
void computePinPosGradLauncher(T const* grad_outs, int32_t const* node_pins,
                               int32_t const* node_pins_start,
                               int32_t num_nodes, int32_t num_threads,
                               T* grads);
...
at::Tensor pinPosForward(at::Tensor pos, at::Tensor pin_offsets,
  ...
  OPENPARF_DISPATCH_FLOATING_TYPES(pos, "computePinPosLauncher", [&] {
...
at::Tensor pinPosBackward(at::Tensor grad_out, at::Tensor pos,
  ...
  OPENPARF_DISPATCH_FLOATING_TYPES(
      grad_out, "computePinPosGradLauncher", [&] {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &OPENPARF_NAMESPACE::pinPosForward, "PinPos forward");
  m.def("backward", &OPENPARF_NAMESPACE::pinPosBackward, "PinPos backward");
}
