...
#include "ops/masked_direct_lg/src/dl_solver.h"
...
void DLSolver::allocateMemory() { _threadMemArray.resize(omp_get_max_threads()); }
  ...
  for (IndexType i : {0, 1}) {
    ...
    for (IndexType j : {0, 1}) {
      ...
      for (IndexType k = beg; k < end; k += _param.BLE_capacity) {
        ...
        // check FF-SHIFT Compatability. FF and SHIFT cannot be placed in the same BLE.
        if (impl.lut[k] != kIndexTypeMax) {
          const auto &lut_inst = _instArray[impl.lut[k]];
          if (lut_inst.type == DLInstanceType::SHIFT || lut_inst.type == DLInstanceType::LRAM) {
            continue;
          }
        }
        if (impl.lut[k ^ 1] != kIndexTypeMax) {
          const auto &lut_inst = _instArray[impl.lut[k ^ 1]];
          if (lut_inst.type == DLInstanceType::SHIFT || lut_inst.type == DLInstanceType::LRAM) {
            continue;
          }
        }
...
bool DLSolver::addLUTToCandidateImpl(const DLInstance &lut, Candidate::Implementation &impl) const {
  ...
  for (IndexType i = 0; i < _param.CLB_capacity; i += _param.BLE_capacity) {
    if (impl.lut[i] == kIndexTypeMax) {
      // check SHIFT-FF Compatability. FF and SHIFT/LRAM cannot be placed in the same BLE.
      if (lut.type == DLInstanceType::LRAM || lut.type == DLInstanceType::SHIFT) {
        if (impl.ff[i] != kIndexTypeMax || impl.ff[i + 1] != kIndexTypeMax) {
          continue;
        }
      }
      impl.lut[i] = lut.id;
...
void DLSolver::printDLStatistics() const {
  ...
  for (auto &p : siteStats) {
    ...
    ss << std::setw(10) << toString(p.first) << ": ";
    auto &st = p.second;
    if (p.first != DLSiteType::DONTCARE && p.first != DLSiteType::OCCUPIED) {
...
void DLSolver::runSlotAssign() {
  ...
  for (IndexType siteId = 0; siteId < numSites(); ++siteId) {
    ...
    if (mem.lut.size() > 0 &&
        (_instArray[mem.lut[0]].type == DLInstanceType::SHIFT || _instArray[mem.lut[0]].type == DLInstanceType::LRAM)) {
      assignLRAMsAndFFs(mem);
    } else {
...
void DLSolver::findBestFFs(const IndexVector     &ff,
  ...
  // LRAM/SHIFT is incompatible with FF in the a BLE.
  if (ble.lut[0] != kIndexTypeMax &&
      (_instArray[ble.lut[0]].type == DLInstanceType::LRAM || _instArray[ble.lut[0]].type == DLInstanceType::SHIFT)) {
    return;
  }
  if (ble.lut[1] != kIndexTypeMax &&
      (_instArray[ble.lut[1]].type == DLInstanceType::LRAM || _instArray[ble.lut[1]].type == DLInstanceType::SHIFT)) {
    return;
  }
  const auto &lutA = _instArray[ble.lut[0]];
...
bool DLSolver::checkFFsToBLEsFeasibility(SlotAssignMemory &mem, std::array<IndexType, 4> &availCE) {
  using Graph      = lemon::SmartDigraph;
  using ArcMap     = lemon::SmartDigraph::ArcMap<int32_t>;
  using NS         = lemon::NetworkSimplex<Graph, int32_t>;

  auto &graph      = *(mem.biGraphPtr);
  auto &leftNodes  = mem.leftNodes;
  auto &rightNodes = mem.rightNodes;
  auto &cksr       = mem.cksr;
  auto &ce         = mem.ce;
  ...
  ArcMap    arcCaps(graph);
  auto      source   = graph.addNode();
  auto      terminal = graph.addNode();

  // Add left nodes
  IndexType n        = mem.ff.size();
  for (IndexType i = 0; i < n; i++) {
    leftNodes.emplace_back(graph.addNode());
    auto &node   = leftNodes.back();
    auto  arc    = graph.addArc(source, node);
    arcCaps[arc] = 1;
  }

  // Add right nodes
  for (IndexType i = 0; i < 4; i++) {
    rightNodes.emplace_back(graph.addNode());
    auto &node   = rightNodes.back();
    auto  arc    = graph.addArc(node, terminal);
    arcCaps[arc] = availCE[i];
  }
  ...
  for (IndexType i = 0; i < n; i++) {
    for (IndexType lh : {0, 1}) {
      for (IndexType k : {0, 1}) {
        auto &ff = _instArray[mem.ff[i]];
        if (ff.cksr == mem.cksr[lh] && ff.ce == mem.ce[2 * lh + k]) {
          auto arc     = graph.addArc(leftNodes[i], rightNodes[2 * lh + k]);
          arcCaps[arc] = 1;
        }
      }
    }
  ...
  // Check feasibility
  NS ns(graph);
  ns.upperMap(arcCaps).stSupply(source, terminal, n);
  NS::ProblemType status = ns.run();
  return status == NS::OPTIMAL;
}

void DLSolver::assignLRAMsAndFFs(SlotAssignMemory &mem) {
  // Don't the existing slot assignment
  // In this way we can also make sure that all LRAM/SHIFT are assigned at odd positions
  ...
  for (IndexType lh : {0, 1}) {
    ...
    for (IndexType idx = beg; idx < end; idx += _param.BLE_capacity) {
      ...
      if (impl.lut[pos] != kIndexTypeMax) {
        std::swap(impl.lut[pos], impl.lut[pos + 1]);
      }
    }
  }
}
...
void DLSolver::assignLUTsAndFFs(SlotAssignMemory &mem) {
  ...
  for (const auto &estBLE : mem.ble) {
    ...
    openparfAssert(lh == 0 ? availLo > 0 : availHi > 0);
  ...
  while (!mem.ff.empty()) {
    ...
    openparfAssertMsg(bestFFIdx != kIndexTypeMax, "Cannnot find feasible position for the rest of unassigned FFs");
