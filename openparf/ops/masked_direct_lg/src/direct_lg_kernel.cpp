...
void initDLProblemNetlist(database::PlaceDB const &db,
  ...
  for (int i = 0; i < num_masked_insts; i++) {
    prob.isInstFixed[masked_inst_ids[i]] = true;
    prob.instTypes[masked_inst_ids[i]]   = DLInstanceType::DONTCARE;
  }
...
void initDLProblemSiteMap(database::PlaceDB const &db,
  ...
  for (int i = 0; i < num_masked_insts; i++) {
    int32_t inst_id      = masked_inst_ids[i];
    int     x            = prob.instXYs[inst_id].x();
    int     y            = prob.instXYs[inst_id].y();
    prob.siteTypes(x, y) = DLSiteType::OCCUPIED;
  }
  ...
  for (auto const &site : layout.siteMap()) {
    ...
    for (auto const &resource : layout.resourceMap()) {
      ...
      if (prob.useXarchLgRule) {
        /* Xarch Benchmark */
        if (prob.siteTypes(x, y) == DLSiteType::OCCUPIED) {
          continue;
        }
...
void initDLProblem(database::PlaceDB const &db, DLProblem &prob, int32_t num_masked_insts, int32_t *masked_inst_ids) {
