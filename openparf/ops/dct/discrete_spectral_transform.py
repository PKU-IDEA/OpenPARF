...
def getExactExpk(N, dtype, device):
...
def getPerm(N, dtype, device):
    ...
    perm[0:(N - 1) // 2 + 1] = torch.arange(0,
                                            N,
                                            2,
                                            dtype=dtype,
                                            device=device)
    perm[(N - 1) // 2 + 1:] = torch.arange(2 * (N // 2) - 1,
                                           0,
                                           -2,
                                           dtype=dtype,
                                           device=device)
...
def dct_2N(x, expk=None):
    ...
    if expk is None:
        expk = getExpk(N, dtype=x.dtype, device=x.device)
...
def dct_N(x, perm=None, expk=None):
    ...
    if perm is None:
        perm = getPerm(N, dtype=torch.int64, device=x.device)
    if x.ndimension() <= 1:
...
def dst(x, expkp1=None):
    ...
    if expkp1 is None:
        expkp1 = getExpkp1(N, dtype=x.dtype, device=x.device)
...
def idxt(x, cos_or_sin_flag, expk=None):
    ...
    # the last dimension here becomes -2 because complex numbers introduce a new dimension
    # Must use IFFT here
    y = torch.ifft(x_pad, signal_ndim=1, normalized=False)[..., 0:N,
                                                           cos_or_sin_flag]
    y.mul_(N)
...
def dct2_2N(x, expk0=None, expk1=None):
    ...
    """
    return dct_2N(
        dct_2N(x.transpose(dim0=-2, dim1=-1), expk0).transpose_(dim0=-2,
                                                                dim1=-1),
        expk1)
...
def dct2_N(x, perm0=None, expk0=None, perm1=None, expk1=None):
    ...
    @param expk1 with length N
    """
    return dct_N(dct_N(x.transpose(dim0=-2, dim1=-1), perm=perm0,
                       expk=expk0).transpose_(dim0=-2, dim1=-1),
                 perm=perm1,
                 expk=expk1)
...
def idct2_2N(x, expk0=None, expk1=None):
    ...
    """
    return idct_2N(
        idct_2N(x.transpose(dim0=-2, dim1=-1), expk0).transpose_(dim0=-2,
                                                                 dim1=-1),
        expk1)
...
def idct2_N(x, expk0=None, expk1=None):
    ...
    """
    return idct_N(
        idct_N(x.transpose(dim0=-2, dim1=-1), expk0).transpose_(dim0=-2,
                                                                dim1=-1),
        expk1)
...
def dst2(x, expkp1_0=None, expkp1_1=None):
    ...
    """
    return dst(
        dst(x.transpose(dim0=-2, dim1=-1), expkp1_0).transpose_(dim0=-2,
                                                                dim1=-1),
        expkp1_1)
...
def idcct2(x, expk_0=None, expk_1=None):
    ...
    """
    return idxt(idxt(x, 0, expk_1).transpose_(dim0=-2, dim1=-1), 0,
                expk_0).transpose(dim0=-2, dim1=-1)
...
def idsct2(x, expk_0=None, expk_1=None):
    ...
    @param expk_1 with length N, 2*exp(-1j*pi*k/(2N))
    """
    return idxt(idxt(x, 0, expk_1).transpose_(dim0=-2, dim1=-1), 1,
                expk_0).transpose_(dim0=-2, dim1=-1)
...
def idcst2(x, expk_0=None, expk_1=None):
    ...
    @param expk_1 with length N, 2*exp(-1j*pi*k/(2N))
    """
    return idxt(idxt(x, 1, expk_1).transpose_(dim0=-2, dim1=-1), 0,
                expk_0).transpose_(dim0=-2, dim1=-1)
    # return idxt(idxt(x.transpose(dim0=-2, dim1=-1), 0, expk_0).transpose_(dim0=-2, dim1=-1), 1, expk_1)


def idxstIdct(x, expk_0=None, expk_1=None):
    '''
    Batch 2D Inverse Discrete Sine-Cosine Transformation without normalization to coefficients.
    ...
    @param expk_1 with length N, 2*exp(-1j*pi*k/(2N))
    '''
    return idxt(idct_N(x, expk_1).transpose_(dim0=-2, dim1=-1), 1,
                expk_0).transpose_(dim0=-2, dim1=-1)


def idctIdxst(x, expk_0=None, expk_1=None):
    '''
    Batch 2D Inverse Discrete Cosine-Sine Transformation without normalization to coefficients.
    ...
    @param expk_1 with length N, 2*exp(-1j*pi*k/(2N))
    '''
    return idct_N(idxt(x, 1, expk_1).transpose_(dim0=-2, dim1=-1),
                  expk_0).transpose_(dim0=-2, dim1=-1)
