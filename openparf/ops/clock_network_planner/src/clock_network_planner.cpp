...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, std::vector<std::vector<int32_t>>, torch::Tensor>
runClockRegionAssignment(database::PlaceDB const &placedb,
    ...
    utplacefx::Parameters::slrAwareFlag                 = pyparam.attr("slrAwareFlag").cast<decltype(utplacefx::Parameters::slrAwareFlag)>();
    ...
    auto instSllOptimMask = torch::zeros({placedb.numInsts()}, options2);
    OPENPARF_DISPATCH_FLOATING_TYPES(pos, "clockNetworkPlannerLauncher", [&] {
        clockNetworkPlannerLauncher<scalar_t>(OPENPARF_TENSOR_DATA_PTR(pos, scalar_t),
                                              ...
                                              OPENPARF_TENSOR_DATA_PTR(instSllOptimMask, uint8_t));
    });
...
//            crs = at::bitwise_and(crs, clkAvailCR.index({ck_id, "..."}));
    ...
    return {nodeToCR, clkAvailCR, instCrAvailMap, instAvailCR, instSllOptimMask};
}
