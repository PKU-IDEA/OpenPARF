...
template <typename T>
static void runClockNetworkAssignment(T const *pos, int32_t num_threads,
                                      int32_t *nodeToCr, uint8_t *clkAvailCR,
                                      T const *inflated_areas,
                                      uint8_t *instSllOptimMask) {
  openparfAssertMsg(false, "Invalid type, only float and double are supported");
}

template <>
void runClockNetworkAssignment<float>(float const *pos, int32_t num_threads,
                                      int32_t *nodeToCr, uint8_t *clkAvailCR,
                                      float const *inflated_areas,
                                      uint8_t *instSllOptimMask) {
...
template <>
void runClockNetworkAssignment<double>(double const *pos, int32_t num_threads,
                                       int32_t *nodeToCr, uint8_t *clkAvailCR,
                                       double const *inflated_areas,
                                       uint8_t *instSllOptimMask) {
...
template <typename T>
void OPENPARF_NOINLINE clockNetworkPlannerLauncher(
    T const *pos, int32_t num_threads, int32_t *nodeToCr, uint8_t *clkAvailCR,
    T const *inflated_areas, uint8_t *instSllOptimMask) {
  runClockNetworkAssignment(pos, num_threads, nodeToCr, clkAvailCR,
                            inflated_areas, instSllOptimMask);
}
