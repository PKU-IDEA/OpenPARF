...
void computeDensityMapLauncher(T const    *pos,
        T const                           *node_sizes,
...
/// @param deterministic_flag whether to use deterministic algorithm
...
void densityMap(at::Tensor          pos,
  ...
  CHECK_FLAT_CPU(bin_map_dims);
  CHECK_CONTIGUOUS(bin_map_dims);
  for (auto const &density_map : density_maps) {
    CHECK_FLAT_CPU(density_map);
    CHECK_CONTIGUOUS(density_map);
  }

  if (range.first < range.second) {
    OPENPARF_DISPATCH_FLOATING_TYPES(pos, "computeDensityMapLauncher", [&] {
      ...
      computeDensityMapLauncher<scalar_t, DensityFunc>(OPENPARF_TENSOR_DATA_PTR(pos, scalar_t),
              ...
              at::get_num_threads(), deterministic_flag, density_map_ptrs);
    });
  }
}
...
template<typename T>
void stretchAreaLauncher(T const          *node_sizes,
        int32_t const                     *bin_map_dims,
        T                                  stretch_ratio,
        T                                  xl,
        T                                  yl,
...
#define CALL_LAUNCHER(range)                                                                                           \
  stretchAreaLauncher<scalar_t>(OPENPARF_TENSOR_DATA_PTR(node_sizes, scalar_t),                                        \
          OPENPARF_TENSOR_DATA_PTR(bin_map_dims, int32_t), stretch_ratio, xl, yl, xh, yh, range, num_area_types,       \
          at::get_num_threads(), OPENPARF_TENSOR_DATA_PTR(node_sizes_stretched, scalar_t),                             \
...
void stretchAreaForward(at::Tensor  node_sizes,
        ...
        at::Tensor                  node_weights) {
  OPENPARF_DISPATCH_FLOATING_TYPES(node_sizes, "stretchAreaLauncher", [&] {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("movableForward", &OPENPARF_NAMESPACE::movableDensityMapForward, "DensityMap for movable and filler cells");
  m.def("fixedForward", &OPENPARF_NAMESPACE::fixedDensityMapForward, "DensityMap for fixed cells");
  m.def("stretchForward", &OPENPARF_NAMESPACE::stretchAreaForward, "Stretch cell sizes");
}
