...
template<typename T>
void ComputeEnergyWellForwardLauncher(const T *inst_pos, const T *half_inst_sizes,
                                      const T *well_boxes, const T *well_energy_function_exponent,
                                      int32_t *selected_crs, T *integral_output,
                                      int32_t *inst_cr_avail_map, int32_t *site2cr_map,
                                      int32_t x_layout_size, int32_t y_layout_size, int32_t num_crs,
                                      int32_t num_insts, int32_t num_threads);
template<typename T>
void ComputeEnergyWellBackwardLauncher(const T *inst_pos, const T *half_inst_sizes,
                                       const T *well_boxes, const int32_t *seleted_crs,
                                       const T *well_energy_function_exponent, const T *grad_output,
...
EnergyWellForward(at::Tensor inst_pos, at::Tensor half_inst_sizes, at::Tensor well_boxes,
    ...
    OPENPARF_DISPATCH_FLOATING_TYPES(inst_pos, "ComputeEnergyWellForwardLauncher", [&] {
        ComputeEnergyWellForwardLauncher<scalar_t>(
                ...
                OPENPARF_TENSOR_DATA_PTR(site2cr_map, int32_t), x_layout_size, y_layout_size,
                num_crs, num_insts, at::get_num_threads());
    });
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &OPENPARF_NAMESPACE::EnergyWellForward, "Energy Well Forward(CUDA)");
    m.def("backward", &OPENPARF_NAMESPACE::EnergyWellBackward, "Energy Well Backward(CUDA)");
}
