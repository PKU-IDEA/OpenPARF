...
std::tuple<at::Tensor, at::Tensor> EnergyWellForward(
    ...
    int32_t num_crs, database::PlaceDB &placedb, at::Tensor cr2slr_map,
    at::Tensor inst2slr_map, at::Tensor inst_cnp_sll_optim_mask) {
  ...
  CHECK_FLAT_CPU(cr2slr_map);
  CHECK_CONTIGUOUS(cr2slr_map);
  CHECK_FLAT_CPU(inst2slr_map);
  CHECK_CONTIGUOUS(inst2slr_map);
  CHECK_FLAT_CPU(inst_cnp_sll_optim_mask);
  ...
  OPENPARF_DISPATCH_FLOATING_TYPES(
      inst_pos, "ComputeEnergyWellForwardLauncher", [&] {
        ComputeEnergyWellForwardLauncher<scalar_t>(
            ...
            xy_to_cr_func, num_crs, num_insts, at::get_num_threads());
      });
...
at::Tensor genCr2SlrMap(database::PlaceDB &placedb) {
  auto options = torch::TensorOptions()
                     .dtype(torch::kInt32)
                     .layout(torch::kStrided)
                     .device(torch::kCPU)
                     .requires_grad(false);
  int32_t    x_cr_size    = placedb.db()->layout().clockRegionMap().width();
  int32_t    y_cr_size    = placedb.db()->layout().clockRegionMap().height();
  at::Tensor cr2slr_map   = at::zeros({x_cr_size * y_cr_size}, options);
  auto       cr2slr_map_a = cr2slr_map.accessor<int32_t, 1>();
  for (int32_t ix = 0; ix < x_cr_size; ix++) {
    for (int32_t iy = 0; iy < y_cr_size; iy++) {
      cr2slr_map_a[ix * y_cr_size + iy] = placedb.CrXyToSlrIndex(ix, iy);
    }
  }
  return cr2slr_map;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("backward", &OPENPARF_NAMESPACE::EnergyWellBackward,
        "Energy Well Backward");
  m.def("genSite2CrMap", &OPENPARF_NAMESPACE::genSite2CrMap,
        "Generate Site to Clock Region Mapping");
  m.def("genCr2SlrMap", &OPENPARF_NAMESPACE::genCr2SlrMap,
        "Generate Clock Region to Super Logic Region Mapping");
}
