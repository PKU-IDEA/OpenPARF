...
inline static T ComputePiecewiseFunction(const T l, const T r, const T exponent,
                                         const T x) {
...
template <typename T,
          typename = typename std::enable_if<std::is_scalar<T>::value>::type>
inline static T ComputePiecewiseFunctionGrad(const T l, const T r,
...
template <typename T,
          typename = typename std::enable_if<std::is_scalar<T>::value>::type>
void ComputeEnergyWellForward(
    const T *inst_pos, const T *half_inst_sizes, const T *well_boxes,
    const T *well_energy_function_exponent, int32_t *selected_crs,
    T *integral_output, int32_t *inst_cr_avail_map, int32_t *cr2slr_map,
    int32_t *inst2slr_map, uint8_t *inst_cnp_sll_optim_mask,
...
#pragma omp parallel for num_threads(num_threads) schedule(dynamic, chunk_size)
  for (int32_t i = 0; i < num_insts; i++) {
    ...
    T       min_dist_withSll    = std::numeric_limits<T>::max();
...
template <typename T,
          typename = typename std::enable_if<std::is_scalar<T>::value>::type>
void ComputeEnergyWellBackward(const T *inst_pos, const T *half_inst_sizes,
                               const T *well_boxes, const int32_t *selected_crs,
                               const T *well_energy_function_exponent,
                               const T *grad_output, T *grad_xy,
...
void OPENPARF_NOINLINE ComputeEnergyWellForwardLauncher(
  ...
  ComputeEnergyWellForward(inst_pos, half_inst_sizes, well_boxes,
                           well_energy_function_exponent, selected_crs,
                           integral_output, inst_cr_avail_map, cr2slr_map,
                           inst2slr_map, inst_cnp_sll_optim_mask, xy_to_cr_func,
...
void OPENPARF_NOINLINE ComputeEnergyWellBackwardLauncher(
  ...
  ComputeEnergyWellBackward(inst_pos, half_inst_sizes, well_boxes, selected_crs,
                            well_energy_function_exponent, grad_output, grad_xy,
                            num_insts, num_threads);
}
