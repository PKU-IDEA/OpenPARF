...
void printRouteResult(std::vector<std::shared_ptr<router::Net>>& netlist, std::string fileName, std::shared_ptr<router::RouteGraph> graph) {
    ...
    for (auto net : netlist) {
        ...
        if (net->getRouteStatus() != router::RouteStatus::SUCCESS) continue;
        ...
        // std::cout << routetree.getTreeNodeByIdx(27655277) << std::endl;
        ...
        for (int i = 0; i < sinkSize; i++) {
            ...
            if (node == nullptr) {
                std::cerr << "[Fatal Error] Nullptr found at Net " << net->getName() << " id: " << pinId << " leaf node " << graph->getVertexByIdx(pinId)->getName() << "!" << std::endl;
                exit(1);
            }
...
void printNetRouteResult(std::shared_ptr<router::Net>& net, std::string fileName, std::shared_ptr<router::RouteGraph> graph, int iter) {
    using std::max;
    using std::min;
    // Pathfinder::routetree.ripup();
    ...
    if (net->getRouteStatus() != router::RouteStatus::SUCCESS) return;
    ...
    file.save_file((net->getName() + "_" + std::to_string(iter) + "_" + fileName).c_str());     
}
